var Bird = function(scene, octree, obstacles, velocity_hash, position, orientation,id) {
	this.scene = scene;
	this.octree = octree;
	...
	//Hacer copy y paste del vector procesado en C.
	this.vectorDatos = new Array([4,3,0x00f91d],[5,3,0x00f9e6],[3,7,0x0004f9],[4,4,0xf90000],[8,15.2566,0x000000],[3,15.2205,0x7e7e7e],[4,13.0247,0xf600ec],[6,13.5213,0xf0ff00]);
...
Bird.prototype.update_position = function(elapsed_time, lookAt) {
	...
	this.distancia+= displacement.length();

	if (elapsed_time/1000 < .5) {
		this.mesh.position.add(displacement);
	}
	//Revisa si es el va mas lento1 que el del ranking
	if(this.velocity.length()<=this.vectorDatos[0][1]){
		this.mesh.material.color.set(this.vectorDatos[0][2]);
		console.log("Mas lento 1");

	}
	//Revisa si es el va mas lento2 que el del ranking
	if(this.velocity.length()<=this.vectorDatos[1][1]){
		this.mesh.material.color.set(this.vectorDatos[1][2]);
		console.log("Mas lento 2");

	}
	//Revisa si es el va mas rapido1 que el del ranking
	if(this.velocity.length()>=this.vectorDatos[2][1]){
		this.mesh.material.color.set(this.vectorDatos[2][2]);
		console.log("Mas rapido 1");

	}
	//Revisa si es el va mas rapido2 que el del ranking
	if(this.velocity.length()>=this.vectorDatos[3][1]){
		this.mesh.material.color.set(this.vectorDatos[3][2]);
		console.log("Mas rapido 2");

	}
	
	//Revisa si es el va mas distancia1 que el del ranking
	if(this.distancia>=this.vectorDatos[4][1]){
		this.mesh.material.color.set(this.vectorDatos[4][2]);
		console.log("mas distancia1");

	}
	//Revisa si es el va mas distancia2 que el del ranking
	if(this.distancia>=this.vectorDatos[5][1]){
		this.mesh.material.color.set(this.vectorDatos[5][2]);
		console.log("mas distancia2");

	}
	//Revisa si es el va menos distancia1 que el del ranking
	if(this.distancia<=this.vectorDatos[6][1]){
		this.mesh.material.color.set(this.vectorDatos[6][2]);
		console.log("menos distancia1");

	}
	//Revisa si es el va menos distancia2 que el del ranking
	if(this.distancia<=this.vectorDatos[7][1]){
		this.mesh.material.color.set(this.vectorDatos[7][2]);
		console.log("menos distancia2");
